...
const Product = ({product}) => {
    ...
    return (
        <div  className='flex h-[400px] md:h-full lg:h-full xl:h-full w-full flex-col items-center justify-between gap-2'>
            ...
            <h1 className='text-2xl md:text-2xl lg:text-xl xl:text-2xl text-black font-bold'>{name}</h1>
        ...
        }} className='px-5 py-2 bg-[#c80042] text-white border-2 rounded-md hover:bg-[#ae1246]  hover:border-[#c80042] hover:text-white '>View Details</button>
            <button onClick={() => {
            navigate(`/update/${_id}`)
        }} className='px-5 py-2 text-black  rounded-md border-black border-2 hover:text-white hover:bg-black '>Update</button>
        </div>
        </div>
    );
};
